...
func getQry(bucket []byte) string {
	return fmt.Sprintf("SELECT nvalue FROM \"%s\" WHERE nkey = $1", bucket)
}

func getQryForUpdate(bucket []byte) string {
	return fmt.Sprintf("SELECT nvalue FROM \"%s\" WHERE nkey = $1 FOR UPDATE", bucket)
}
...
func insertUpdateQry(bucket []byte) string {
	return fmt.Sprintf(`
		...
		VALUES($1,$2)
		ON CONFLICT (nkey) DO UPDATE
		SET nvalue = excluded.nvalue
	`, bucket)
}

func delQry(bucket []byte) string {
	return fmt.Sprintf("DELETE FROM \"%s\" WHERE nkey = $1", bucket)
}

func createTableQry(bucket []byte) string {
	return fmt.Sprintf("CREATE TABLE IF NOT EXISTS \"%s\"(nkey BYTEA, nvalue BYTEA, PRIMARY KEY (nkey));", bucket)
}
